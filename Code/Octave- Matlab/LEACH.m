...
save('leach.mat', 'ENERGY', 'PACKETS_TO_BS', 'STATISTICS.ALLIVE');
save('leach2.mat', 'STATISTICS');

r=0:rmax;
figure(1)
plot(r,STATISTICS.ALLIVE(h+1,r+1));
xlim([0 1500]);
title('Alive nodes','fontsize',10)
xlabel('x (No. of rounds)', 'fontsize',10);
ylabel('y (Alive nodes)','fontsize',10);

figure(2)
plot(r,STATISTICS.ENERGY(h+1,r+1));
xlim([0 1500]);
xlabel('x (No. of rounds)','fontsize',10);
ylabel('y (Residual Energy)','fontsize',10);
title('Energy remaining in network','fontsize',10)

figure(3)
plot(r,STATISTICS.PACKETS_TO_BS(h+1,r+1));
xlim([0 1500]);
xlabel('x (No. of rounds)','fontsize',10);
ylabel('y (Data packets sent to BS)','fontsize',10);
title('Packets to BS','fontsize',10)
