...
leachENERGY = load('leachEnergy.mat');
leachDataENERGY = leachENERGY.ENERGY;
leachPACKETS_TO_BS = load('leachPckts.mat');
leachDataPACKETS_TO_BS = leachPACKETS_TO_BS.PACKETS_TO_BS;
leachAlive = load('leachAlive.mat');
leachDataAlive = leachAlive.ALLIVE;
...
% doo not transmit (increment number of packets) if node is malicious 
...
if ((distance>doo) &&(i~=3)&&(i~=5)&&(i~=6)&&(i~=7) &&(i~=8)&&(i~=10)&&(i~=3)&&(i~=65)&&(i~=16)&&(i~=37) &&(i~=38)&&(i~=50))
S3(i).E=S3(i).E- 1.5*( (ETX+EDA)*(4000) + Emp*4000*( distance*distance*distance*distance )); 
end
            if ((distance<=doo) &&(i~=3)&&(i~=5)&&(i~=6)&&(i~=7) &&(i~=8)&&(i~=10)&&(i~=3)&&(i~=65)&&(i~=16)&&(i~=27) &&(i~=38)&&(i~=50))
...
al = figure(1)
plot(r,STATISTICS.ALLIVE2,'--',r,STATISTICS.ALLIVE3,'--', r, leachDataAlive);
xlim([0 1500]);
%ylim([-0.4 0.8]);
legend('With sinkhole attack','Withblackhole attack','Without Attack');
xlabel('x( No. of rounds)','fontsize',12);
ylabel('y (Alive nodes)','fontsize',12);
title('Nodes alive during rounds','fontsize',12);
%print (al, "alive.pdf", "-dpdflatexstandalone");

pk = figure(2)
plot(r,STATISTICS.PACKETS_TO_BS2,'--',r,STATISTICS.PACKETS_TO_BS3,'--', r, leachDataPACKETS_TO_BS);
xlim([0 1500]);
legend('With sinkhole attack','Withblackhole attack', 'Without attack');
xlabel('x (No. of rounds)','fontsize',12);
ylabel('y (Data transmitted)','fontsize',12);
title('Packets sent to base station','fontsize',12);
%print (pk, "packets.pdf", "-dpdflatexstandalone");


en = figure(3)
plot(r,El2,'--',r,El3,'--',r,leachDataENERGY);
xlim([0 1500]);
ylim([0 50]);
legend('With sinkhole attack','With blackhole attack','Without attack');
xlabel('x (No. of rounds)','fontsize',12);
ylabel('y (Residual energy)','fontsize',12);
title('Energy left in the network','fontsize',12);
%print (en, "energy.pdf", "-dpdflatexstandalone");
